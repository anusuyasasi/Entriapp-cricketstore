...
const slides = [
  {
    ...
    saleText: "Get in the game with up to 50% off on a wide range of premium cricket equipment",
  ...
  },
  {
    ...
    saleText: "Don't miss out on the opportunity to upgrade your game with professional-grade gear",
    productText: "Buy Now",
  },
  {
    image: img01,
...
export default function HeroSlider() {
  ...
  const handleNext = () => setActiveStep((prev) => (prev + 1) % slides.length);
  const handleBack = () => setActiveStep((prev) => (prev - 1 + slides.length) % slides.length);
  ...
  return (
    <div className={classes.heroContainer}>
      <Carousel
        ...
        navButtonsProps={{
          style: { backgroundColor: "transparent", padding: 0, margin: "0 20px" },
        }}
            ...
            <AnimatePresence mode="wait">
              {activeStep === index && (
                <motion.div
                  ...
                  exit="hidden"
                >
                  <motion.p className={classes.tagline} variants={itemVariants}>{slide.tagline}</motion.p>
                  <motion.h1 className={classes.quote} variants={itemVariants}>{slide.quote}</motion.h1>
                  <motion.p className={classes.saleText} variants={itemVariants}>{slide.saleText}</motion.p>
